...
class CadastroEquipamentosScreen(Screen):
    ...
    def voltar_inserir_nota_fiscal(self, instance):
        self.manager.current = 'cadastro_equipamento1'  # Mudei para o nome correto da tela
...
class CadastroEquipamentosScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Botão 'Voltar' no canto inferior esquerdo
        voltar_button = Button(text='Voltar', size_hint=(None, None), size=(100, 30), pos_hint={'x': 0.05, 'y': 0.05}, background_color=(0.8, 0.2, 0.2, 0.5))  # Cor padrão (branco)
        voltar_button.bind(on_press=self.voltar_para_cadastro_normal)
        self.layout.add_widget(voltar_button)

        self.add_widget(self.layout)

    def voltar_para_cadastro_normal(self, instance):
        self.manager.current = 'cadastro_equipamentos'

    def salvar_os(self, instance):
        self.manager.current = 'cadastro_equipamento1'
...
class TelaCadastroEquipamento1(Screen):
    def __init__(self, **kwargs):
        ...
        botao_voltar = Button(text='Voltar', size_hint=(None, None), size=(100, 30),
                              pos=(50, 50),  # Posiciona no canto inferior esquerdo
                              background_normal='', background_color=(0.2, 0.6, 0.9, 1), color=(1, 1, 1, 1))
        botao_voltar.bind(on_press=self.voltar_para_anterior)
    ...
    def voltar_para_anterior(self, instance):
        # Implemente aqui a lógica para voltar para a tela anterior
        app = App.get_running_app()
        app.root.current = 'cadastro'  # Substitua 'tela_anterior' pelo nome da tela anterior

class TelaAnterior(Screen):
    pass
...
class ACCIApp(App):
    ...
    def build(self):
        ...
        gerenciador.add_widget(TelaCadastroEquipamento1(name='cadastro_equipamento1'))
